...
export class Helper {
    ...
    ajax(args) {
        return new Promise((resolve, reject) => {
            const controller = typeof args.controller === 'undefined' ? 'php/controller2.php' : args.controller;
            const token = typeof args.token !== 'undefined' ? `&t=${args.token}` : `&t=${objGameLayout.token}`;
            const kind = typeof args.kind === 'undefined' ? 'POST' : args.kind;
